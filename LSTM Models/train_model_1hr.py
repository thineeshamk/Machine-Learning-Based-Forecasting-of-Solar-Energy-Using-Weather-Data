...
# *** NEW FOLDER FOR 1-HOUR MODEL ***
OUTPUT_FOLDER = 'model_artifacts_1hr'

# Model Hyperparameters for 1 HR Model
SEQUENCE_LENGTH = 60   # Past 60 time steps (1 hour)
PREDICTION_LENGTH = 60 # Future 60 steps to predict (1 hour)
DROPOUT_RATE = 0.2
LSTM_UNITS = 50
EPOCHS = 50
BATCH_SIZE = 32
...
def main():
    print("--- Starting 1-Hour Model Training Pipeline ---")
    ...
    # 3. Save Scalers
    # Saving to the new 'model_artifacts_1hr' folder
    feature_scaler_path = os.path.join(OUTPUT_FOLDER, 'feature_scaler_1hr.pkl')
    target_scaler_path = os.path.join(OUTPUT_FOLDER, 'target_scaler_1hr.pkl')
    ...
    # 4. Prepare Sequences
    print("Creating sequences (60 steps)...")
    ...
    # 6. Save Model
    # Saving to the new 'model_artifacts_1hr' folder
    model_path = os.path.join(OUTPUT_FOLDER, 'lstm_model_1hr.h5')
    ...
    plt.title('1-Hour Model Training Progress')
    plt.ylabel('Loss')
    plt.xlabel('Epoch')
    plt.legend()
    plt.show()
